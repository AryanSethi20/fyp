...
class TrainingMetrics:
    ...
    def save_metrics(self, filename="training_metrics-copy.json"):
        metrics = {
            "rewards": {
...
def print_training_summary(metrics):
    print("\nTraining Summary")
    print("=" * 50)
    print(f"Total Episodes: {len(metrics.episode_rewards) / 50}")
...
def train_rl_agent(env, agent, episodes=300, batch_size=32, window_size=5, convergence_threshold=0.01, run=5):
...
def train_double_dqn_agent(env, agent, episodes=300, batch_size=32, window_size=5, 
                          target_update_freq=10, save_interval=10, run=5):
    """
